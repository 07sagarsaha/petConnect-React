...
const PetModel = ({ isAnimating, showCanvas, register, login }) => {
  ...
  useFrame((state, delta) => {
    ...
    if (!isAnimating) {
      ...
      modelRef.current.position.lerp(new THREE.Vector3(0, -2, 0), 0.05);
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className="fixed z-50 w-fit backdrop-blur-md p-4 shadow-lg rounded-r-full"
          onClick={() => window.location.reload()}
        >
          <Header />
        </div>
        <button
          className="btn btn-primary text-base-100 fixed w-fit top-4 right-8 z-50"
          onClick={handleDirectShowLogin}
        >
          {"Login"}
        </button>
        <div className="flex flex-col bg-base-100 relative min-h-[200vh]">
          ...
          {/* Welcome Section 1 */}
          <div
            className={`flex items-start my-32 justify-center px-8 flex-grow relative z-20 pointer-events-none transition-transform duration-1000 ease-in-out ${showCanvas ? "-translate-y-[2000px]" : ""}`}
          >
            <div className="w-full max-w-2xl px-4 py-16 rounded-xl animate-postButtonAnim1 backdrop-blur-md bg-base-100/50 shadow-lg pointer-events-auto">
              ...
              <p className="text-lg mb-4 text-center italic">{"Guess what?"}</p>
          ...
          <div className="flex flex-col">
            ...
            {/* Register Section */}
            {register && (
              <div className="fixed inset-y-0 left-15 top-36 w-1/2 h-fit max-sm:w-full z-30 flex items-center justify-center">
