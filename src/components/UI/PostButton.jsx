...
const Button = ({buttonName}) => {
  ...
  return (
    <>
    <button className={`text-3xl absolute left-[1050px] top-10 rounded-lg p-4 hover:bg-red-500 animate-postButtonAnim2 hover:text-white transition-all duration-500 ${!isClicked && `hidden`}`} onClick={handleClickEvent}>{isClicked && 'x'}</button>
    <div id='submit' onClick={isClicked ? null : handleClickEvent} disabled={isClicked} className={`text-xl p-6 m-8 flex justify-center items shadow-xl h-max min-h-12 w-max min-w-20 rounded-2xl bg-gradient-to-b from-[#c183f8c4] to-[#f173bac0] text-white hover:shadow-2xl border-4 transition-all ease-in-out animate-postButtonAnim1 duration-700 ${isClicked && `min-w-[55rem] min-h-[42rem] text-[200%]`}`}>
      {"New Post"}
